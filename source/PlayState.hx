...
class PlayState extends MusicBeatState
{
	...
	public var playingAsOpp:Bool = false;
	public var multiplayer:Bool = false;
	...
	override public function create()
	{
			...
			if(FileSystem.exists(folder))
				{
					for (file in FileSystem.readDirectory(folder))
					{
						if(file.endsWith('.hx'))
						{
							scripts.addScript(folder+file);
						}
					}
				}
		}

		FlxG.autoPause = !multiplayer;
		...
		var file = Paths.getPreloadPath('stages/' + curStage + '.hx');
		if(FileSystem.exists(file)) {
			scripts.addScript(file);
		}
		...
		if(playingAsOpp)
		{
			...
			for(i in 0...opponentStrums.length)
				opponentStrums.members[i].x = ppPos[i];
			for(i in 0...playerStrums.length)
				playerStrums.members[i].x = oppPos[i];
		}
	...
	function startNextDialogue() {
		...
		scripts.call("onNextDialogue");
	}

	function skipDialogue() {
		callOnLuas('onSkipDialogue', [dialogueCount]);
		scripts.call("onSkipDialogue");
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (controls.PAUSE && startedCountdown && canPause && !multiplayer)
		{
		...
		if (FlxG.keys.anyJustPressed(debugKeysChart) && !endingSong && !inCutscene && !multiplayer)
		{
			openChartEditor();
		}
		...
		if (FlxG.keys.anyJustPressed(debugKeysCharacter) && !endingSong && !inCutscene && !multiplayer) {
		...
		if (!ClientPrefs.noReset && controls.RESET && canReset && !inCutscene && startedCountdown && !endingSong && !multiplayer)
		{
			health = 0;
			trace("RESET = True");
		}
		...
		if (unspawnNotes[0] != null)
		{
			...
			while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				...
				scripts.call("onSpawnNote",[notes.members.indexOf(dunceNote), dunceNote.noteData, dunceNote.noteType, dunceNote.isSustainNote]);
		...
		#if debug
		if(!endingSong && !startingSong && !multiplayer) {
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		scripts.call("noteMiss", [notes.members.indexOf(daNote), daNote.noteData, daNote.noteType, daNote.isSustainNote]);
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		scripts.call("opponentNoteHit", [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote]);
